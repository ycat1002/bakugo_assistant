...
✚ ["+t.category+"] "+t.task+(t.date?" "+t.date:"")}]);
  ...
  const completeTask = async (task) => {
    ...
    setCP(false);
    setMsgs(p=>[...p,{role:"assistant",text:wrap("よし。(좋아.)")+"\n\n✅ \""+task.task+"\" 완료."}]);
            ...
            {completePicker&&(
              <div style={{background:C.winDim,border:`2px solid ${C.lavender}`,padding:"10px 12px"}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:6}}>
                  {pending.map((t,i)=>(
                    <button key={t.id||i} onClick={()=>completeTask(t)} style={{padding:"8px 12px",border:`1.5px solid ${C.border}`,background:C.win,fontSize:13,fontWeight:700,color:C.text,cursor:"pointer",textAlign:"left",fontFamily:C.ss}}>⬜ [{t.category}] {t.task}</button>
                  ))}
